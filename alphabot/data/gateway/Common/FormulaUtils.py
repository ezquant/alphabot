...
def HHV(DF: pd.DataFrame, N):
    return pd.Series.rolling(DF, N).max()


def LLV(DF: pd.DataFrame, N):
    return pd.Series.rolling(DF, N).min()


def SUM(DF: pd.DataFrame, N):
...
def REF(DF: pd.DataFrame, N):
    var = DF.diff(N)
    var = DF - var
    return var


def STD(DF: pd.DataFrame, N):
    return pd.Series.rolling(DF, N).std()


def MACD(DF: pd.DataFrame, FAST, SLOW, MID):
    EMAFAST = EMA(DF, FAST)
    EMASLOW = EMA(DF, SLOW)
...
def OSC(DF: pd.DataFrame, N, M):  # 变动速率线
    C = DF['close']
    OS = (C - MA(C, N)) * 100
    MAOSC = EMA(OS, M)
...
def BBI(DF: pd.DataFrame, N1, N2, N3, N4):  # 多空指标
    C = DF['close']
...
def BBIBOLL(DF: pd.DataFrame, N1, N2, N3, N4, N, M):  # 多空布林线
...
def PBX(DF: pd.DataFrame, N1, N2, N3, N4, N5, N6):  # 瀑布线
    C = DF['close']
...
def BOLL(DF: pd.DataFrame, N):  # 布林线
    C = DF['close']
    boll = MA(C, N)
    UB = boll + 2 * STD(C, N)
    LB = boll - 2 * STD(C, N)
...
def ROC(DF: pd.DataFrame, N, M):  # 变动率指标
    C = DF['close']
...
def MTM(DF: pd.DataFrame, N, M):  # 动量线
    C = DF['close']
    mtm = C - REF(C, N)
    MTMMA = MA(mtm, M)
...
def MFI(DF: pd.DataFrame, N):  # 资金指标
    C = DF['close']
    H = DF['high']
    L = DF['low']
    VOL = DF['vol']
...
def WR(DF: pd.DataFrame, N, N1):  # 威廉指标
    HIGH = DF['high']
    LOW = DF['low']
    CLOSE = DF['close']
...
def BIAS(DF: pd.DataFrame, N1, N2, N3):  # 乖离率
    CLOSE = DF['close']
...
def RSI(DF: pd.DataFrame, N1, N2, N3):  # 相对强弱指标RSI1:SMA(MAX(CLOSE-LC,0),N1,1)/SMA(ABS(CLOSE-LC),N1,1)*100;
    CLOSE = DF['close']
    LC = REF(CLOSE, 1)
...
def ADTM(DF: pd.DataFrame, N, M):  # 动态买卖气指标
    HIGH = DF['high']
    LOW = DF['low']
    OPEN = DF['open']
...
def DDI(DF: pd.DataFrame, N, N1, M, M1):  # 方向标准离差指数
    H = DF['high']
    L = DF['low']
