'''
聚宽社区文章：[借助JqData搭建简易回测框架]
(https://www.joinquant.com/view/community/detail/be9734dde696ca8389c8b66d323d6586?type=1)

下面的代码是一个简单的回测模块，数据需要引用聚宽的 jqdata。
把下面的代码保存为一个.py 文件，然后上传到聚宽研究根目录下（或者你研究所在文件目录下），
然后使用from 文件名 import *，即可导入该模块。

具体方法请参考：https://hicoder.com.cn/2020/05/04/KDJ指标在指数上的择时效应/
'''
