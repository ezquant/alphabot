...
class ZTClientTrader():
	...
	def __init__(self, broker_type='ths'):
		self._broker_type = broker_type
		self._config = create(self._broker_type)
		self._app = None
		self._main = None
	...
	def login(self, account, password):
		...
		# 选择营业部
		#self._main.ComboBox2.select(15) #中信建投客户端无营业部选项
		...
		code = get_yzm_text(yzm_control)
		code = code[:4] #本程序验证码只有四位，识别程序返回值有时会多出一位

		wins = self._main.children(class_name='Edit')[6]
		try: #有时界面不显示验证码输入框，这里就会报错 ElementNotVisible
			wins.type_keys(code)
